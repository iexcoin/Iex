                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zIEX ought to be enough for anybody." - Bill Gates, 2017
    ui->zIEXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzIEXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzIEX->setEnabled(false);
        ui->pushButtonMintzIEX->setToolTip(tr("zIEX is currently disabled due to maintenance."));

        ui->pushButtonSpendzIEX->setEnabled(false);
        ui->pushButtonSpendzIEX->setToolTip(tr("zIEX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zIEXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzIEX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIEX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zIEX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zIEX in ") +
...
void PrivacyDialog::on_pushButtonSpendzIEX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zIEX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zIEX
        sendzIEX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zIEX
    sendzIEX();
}

void PrivacyDialog::on_pushButtonZIEXControl_clicked()
{
    ZIEXControlDialog* zIEXControl = new ZIEXControlDialog(this);
    zIEXControl->setModel(walletModel);
    zIEXControl->exec();
}

void PrivacyDialog::setZIEXControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzIEXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzIEX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid IEX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zIEXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zIEX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " IEX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zIEX</b>";
    ...
    // use mints from zIEX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZIEXControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZIEXControlDialog::GetSelectedMints();
    }
    ...
    // Clear ziex selector in case it was used
    ZIEXControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zIEX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " IEX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zIEX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zIEX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zIEX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zIEX "));
    ui->labelzIEXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzIEXSyncStatus->setVisible(fShow);
}
