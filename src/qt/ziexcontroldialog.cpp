...
#include "ui_ziexcontroldialog.h"
...
std::list<std::string> ZIEXControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZIEXControlDialog::listMints;

ZIEXControlDialog::ZIEXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZIEXControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZIEXControlDialog::~ZIEXControlDialog()
{
    delete ui;
}

void ZIEXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIEXControlDialog::updateList()
{
...
void ZIEXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIEXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZIEX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIEXControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZIEXControlDialog::GetSelectedMints()
{
...
void ZIEXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
