...
*Setup instructions for a gitian build of IEX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IEX
...
Clone the git repositories for iex and gitian and then checkout the iex version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the iex repository
...
To build IEX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the iex repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/iex/.git/
...
./bin/gbuild --commit iex=${COMMIT} --url iex=${URL} ../IEX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit iex=${COMMIT} --url iex=${URL} ../IEX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit iex=${COMMIT} --url iex=${URL} ../IEX/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/iex-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/iex-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/iex-build.assert
```
